...
namespace VL\LMS\Tests\Unit\Admin\Topics;
...
final class TopicPickerAjaxHandlerTest extends TestCase {
	...
	private static function fakeTopic( int $id, int $parent_id, string $title ): WP_Post {
	...
	public function test_search_sends_403_on_bad_nonce(): void {
		...
		$this->invoke( fn () => ( new TopicPickerAjaxHandler() )->search() );

		self::assertCount( 1, $this->error_calls );
		self::assertSame( 403, $this->error_calls[0][1] );
	}

	public function test_search_returns_mapped_unattached_topics(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_posts' )->justReturn(
			[
				self::fakeTopic( 5, 0, 'Alpha' ),
				self::fakeTopic( 6, 0, 'Beta' ),
			]
		);
	...
	public function test_attach_rejects_already_attached_topic(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_lesson' );
		Functions\when( 'get_post' )->justReturn( self::fakeTopic( 9, 123, 'Owned' ) );
		...
		$this->invoke( fn () => ( new TopicPickerAjaxHandler() )->attach() );
	...
	public function test_attach_rejects_parent_that_is_not_a_lesson(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
	...
	public function test_attach_rejects_non_topic(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_lesson' );
		$lesson            = Mockery::mock( 'WP_Post' );
		$lesson->ID        = 9;
		$lesson->post_type = 'vl_lesson';
		Functions\when( 'get_post' )->justReturn( $lesson );
	...
	public function test_attach_writes_parent_and_menu_order(): void {
		...
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_lesson' );
		Functions\when( 'get_post' )->justReturn( self::fakeTopic( 9, 0, 'Free topic' ) );

		// next_menu_order() lookup returns one existing sibling at order 3.
		Functions\when( 'get_posts' )->justReturn( [ self::fakeTopicWithOrder( 4, 3 ) ] );
	...
	public function test_detach_zeroes_parent(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_topic' );
		...
		$this->invoke( fn () => ( new TopicPickerAjaxHandler() )->detach() );
	...
	private static function fakeTopicWithOrder( int $id, int $order ): WP_Post {
		$post             = self::fakeTopic( $id, 42, 'Sibling' );
