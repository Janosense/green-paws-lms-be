...
namespace VL\LMS\Tests\Unit\Admin\Modules;
...
final class ModulePickerAjaxHandlerTest extends TestCase {
	...
	private static function fakeModule( int $id, int $parent_id, string $title ): WP_Post {
	...
	public function test_search_sends_403_on_bad_nonce(): void {
		...
		$this->invoke( fn () => ( new ModulePickerAjaxHandler() )->search() );

		self::assertCount( 1, $this->error_calls );
		self::assertSame( 403, $this->error_calls[0][1] );
	}

	public function test_search_returns_mapped_unattached_modules(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_posts' )->justReturn(
			[
				self::fakeModule( 5, 0, 'Alpha' ),
				self::fakeModule( 6, 0, 'Beta' ),
			]
		);
	...
	public function test_attach_rejects_already_attached_module(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_course' );
		Functions\when( 'get_post' )->justReturn( self::fakeModule( 9, 123, 'Owned' ) );
		...
		$this->invoke( fn () => ( new ModulePickerAjaxHandler() )->attach() );
	...
	public function test_attach_rejects_non_module(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_course' );
	...
	public function test_attach_writes_parent_and_menu_order(): void {
		...
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_course' );
		Functions\when( 'get_post' )->justReturn( self::fakeModule( 9, 0, 'Free module' ) );

		// next_menu_order() lookup returns one existing sibling at order 3.
		Functions\when( 'get_posts' )->justReturn( [ self::fakeModuleWithOrder( 4, 3 ) ] );
	...
	public function test_detach_zeroes_parent(): void {
		...
		$this->invoke( fn () => ( new ModulePickerAjaxHandler() )->detach() );
	...
	private static function fakeModuleWithOrder( int $id, int $order ): WP_Post {
		$post             = self::fakeModule( $id, 42, 'Sibling' );
