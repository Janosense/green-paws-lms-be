...
namespace VL\LMS\Tests\Unit\Admin\Questions;
...
final class QuestionPickerAjaxHandlerTest extends TestCase {
	...
	private static function fakeQuestion( int $id, int $parent_id, string $title ): WP_Post {
	...
	public function test_search_sends_403_on_bad_nonce(): void {
		...
		$this->invoke( fn () => ( new QuestionPickerAjaxHandler() )->search() );

		self::assertCount( 1, $this->error_calls );
		self::assertSame( 403, $this->error_calls[0][1] );
	}

	public function test_search_returns_mapped_unattached_questions(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_posts' )->justReturn(
			[
				self::fakeQuestion( 5, 0, 'Alpha' ),
				self::fakeQuestion( 6, 0, 'Beta' ),
			]
		);
	...
	public function test_attach_rejects_already_attached_question(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_quiz' );
		Functions\when( 'get_post' )->justReturn( self::fakeQuestion( 9, 123, 'Owned' ) );
		...
		$this->invoke( fn () => ( new QuestionPickerAjaxHandler() )->attach() );
	...
	public function test_attach_rejects_parent_that_is_not_a_quiz(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
	...
	public function test_attach_rejects_non_question(): void {
		Functions\when( 'wp_verify_nonce' )->justReturn( 1 );
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_quiz' );
		$quiz            = Mockery::mock( 'WP_Post' );
		$quiz->ID        = 9;
		$quiz->post_type = 'vl_quiz';
		Functions\when( 'get_post' )->justReturn( $quiz );
	...
	public function test_attach_writes_parent_and_menu_order(): void {
		...
		Functions\when( 'current_user_can' )->justReturn( true );
		Functions\when( 'get_post_type' )->justReturn( 'vl_quiz' );
		Functions\when( 'get_post' )->justReturn( self::fakeQuestion( 9, 0, 'Free question' ) );

		// next_menu_order() lookup returns one existing sibling at order 3.
		Functions\when( 'get_posts' )->justReturn( [ self::fakeQuestionWithOrder( 4, 3 ) ] );
	...
	public function test_detach_zeroes_parent(): void {
		...
		$this->invoke( fn () => ( new QuestionPickerAjaxHandler() )->detach() );
	...
	private static function fakeQuestionWithOrder( int $id, int $order ): WP_Post {
		$post             = self::fakeQuestion( $id, 42, 'Sibling' );
